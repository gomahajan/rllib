...
parser.add_argument('--buffer-size', type=int, default=2, metavar='N',
...
parser.add_argument('--min-v-loss', type=int, default=10, metavar='N',
                    help='min loss for v function (default: 1)')
...
def learn_q(dataset):
    ...
    optimizer = optim.RMSprop(qfunction.parameters(), args.lr, eps=args.eps, alpha=args.alpha)
...
def learn_v(dataset):
    ...
    optimizer = optim.RMSprop(vfunction.parameters(), 1e-2, eps=args.eps, alpha=args.alpha)
    ...
    for epoch in count(1):
        ...
        for i, data in enumerate(trainloader, 0):
            ...
            for state, action, reward_to_go in zip(states, actions, reward_to_gos):
                losses.append((vfunction(state) - reward_to_go) ** 2)
        ...
        if args.debug:
            plt.figure(6)
            plt.clf()
            plt.title('VFunction Loss')
            plt.plot(vloss)
            plt.pause(0.001)

        if loss <= 10 or epoch % 4 == 0:
            break


# learn best policy based on q value
ploss = []
policy_optimizer = optim.RMSprop(policy.parameters(), args.lr, eps=args.eps, alpha=args.alpha)
    ...
    for epoch in count(1):
        for i, data in enumerate(trainloader, 0):
            ...
            for state, action, reward_to_go in zip(states, actions, reward_to_gos):
                ...
                losses.append(-log_prob_action(state, action) * (reward_to_go-vfunction(state)))  # qfunction(state)[int(action.item())]
...
def createDataset(experiences):
    ...
    return ExperienceDataset(torch.Tensor(states), torch.Tensor(actions).view(-1, 1),
                             torch.Tensor(reward_to_gos).view(-1, 1))
...
def main():
    ...
    for step in count(1):
        new_exp = sample()
        update_exp(new_exp)
        dataset = createDataset(exps)
        if args.approx:
            learn_q(dataset)
        learn_policy(dataset)
        learn_v(createDataset([new_exp]))
        ...
        if running_reward > env.spec.reward_threshold:
            print("Solved! Running reward is now {} and "
                  "the last episode runs to {} time steps!".format(running_reward, new_exp.value))
            break
